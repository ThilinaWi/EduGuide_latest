...
start "API Gateway" cmd /k "cd backend\api-gateway && python -m pip install -r requirements.txt -q && python main.py"
timeout /t 2

REM Start Adaptive Learning Backend
echo Starting Adaptive Learning Backend on port 5001...
start "Adaptive Learning Backend" cmd /k "cd backend\adaptive-learning && python -m pip install -r requirements.txt -q && python main.py"
timeout /t 2

REM Start Risk Predictor Backend
echo Starting Risk Predictor Backend on port 5002...
start "Risk Predictor Backend" cmd /k "cd backend\risk-predictor && pip install -r requirements.txt -q && python app.py"
timeout /t 2

REM Start Stress Prediction Backend
echo Starting Stress Prediction Backend on port 5003...
start "Stress Prediction Backend" cmd /k "cd backend\stress-prediction && pip install -r requirements.txt -q && python app.py"
timeout /t 2
